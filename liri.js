...
const concertThis = async (band = process.argv[3]) => {
  if (process.argv[4]) {
    console.log(
      chalk.bgRed('Try entering the artist with quotes ("") surrounding it.')
    );
    return;
  }
  ...
  try {
    ...
    response.data.forEach(concert => {
      ...
      fs.appendFile(
        "log.txt",
        `| Venue - ${concert.venue.name}, Location - ${
          concert.venue.city + ", " + concert.venue.region
...
const spotifyThisSong = async (song = process.argv[3]) => {
  if (process.argv[4]) {
    console.log(
      chalk.bgRed('Try entering the song with quotes ("") surrounding it.')
    );
    return;
  }
  if (!song) {
  ...
  spotify.search({ type: "track", query: song }, function(err, data) {
    ...
    console.log(
      chalk.magenta("Listen URL:"),
      chalk.bgCyan(song.external_urls.spotify)
    );
    console.log(chalk.green("Album:"), song.album.name);
    fs.appendFile(
      "log.txt",
      `Song Name - ${song.name}, Artist(s) - ${
        song.artists[0].name
      }, Listen URL - ${song.external_urls.spotify}, Album - ${
        song.album.name
      }]`,
      err => {
        if (err) throw err;
...
const movieThis = async (input = process.argv[3]) => {
  ...
  try {
    ...
    fs.appendFile(
      "log.txt",
      `Year - ${movie.Year}, IMDB - ${
        movie.Ratings[0].Value
      }, Rotten Tomatoes - ${movie.Ratings[1].value}, Country - ${
        movie.Country
      }, Language - ${movie.Language}, Plot - ${movie.Plot}, Actors ${
        movie.Actors
      }]`,
      err => {
        if (err) throw err;
